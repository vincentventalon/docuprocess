/**
 * API Route: Download QR Code Sheet PDF
 * POST /api/generators/qr-code-sheet/download
 *
 * Generates and returns PDF for a QR code sheet template
...
const TOOL_NAME = "qr-code-sheet-generator";
  ...
  try {
    ...
    return new NextResponse(pdfBytes, {
      ...
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="qr-code-sheet.pdf"',
        "X-Downloads-Remaining": String(downloadsRemaining - 1),
      },
    });
  } catch (error) {
    console.error("Error generating QR code sheet PDF:", error);
    return NextResponse.json(
