/**
 * API Route: Download Barcode Sheet PDF
 * POST /api/generators/barcode-sheet/download
 *
 * Generates and returns PDF for a barcode sheet template
...
const TOOL_NAME = "barcode-sheet-generator";
  ...
  try {
    ...
    return new NextResponse(pdfBytes, {
      ...
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="barcode-sheet.pdf"',
        "X-Downloads-Remaining": String(downloadsRemaining - 1),
      },
    });
  } catch (error) {
    console.error("Error generating barcode sheet PDF:", error);
    return NextResponse.json(
